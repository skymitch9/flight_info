...
- [x] 1. Create Route model and update existing models
  - [x] 1.1 Add Route SQLAlchemy model to `app/models.py`
  ...
  - [x] 1.2 Modify TripRequest model with route_id, status, fulfilled_at
  ...
  - [x] 1.3 Modify PriceSnapshot model with route_id, make trip_request_id nullable
...
- [x] 2. Implement RouteTracker service
  - [x] 2.1 Create `app/route_tracker/__init__.py` and `app/route_tracker/service.py`
...
- [x] 3. Modify TripManager for fulfillment and route linking
  - [x] 3.1 Update `app/trip_manager/service.py` — integrate route linking on trip creation
  ...
  - [x] 3.2 Add `fulfill_trip(trip_id)` method to TripService
  ...
  - [x] 3.3 Add `list_fulfilled_trips()` method to TripService
    - Query TripRequests where `status == "fulfilled"`, load relationships
    - _Requirements: 2.4, 4.1_

- [x] 4. Refactor CollectionService to route-level iteration
  - [x] 4.1 Modify `app/collector/service.py` to iterate routes
...
- [x] 6. Update GraphQL schema with new queries and mutations
  - [x] 6.1 Add `RouteType` to GraphQL schema
    - Define `RouteType` with fields: id, origin, destination, status, lastCollectedAt, priceHistory, activeContracts
    - _Requirements: 5.1, 5.2_

  - [x] 6.2 Add `fulfilledTrips` query
  ...
  - [x] 6.3 Add `route(routeId: Int!)` and `routes` queries
    - `route` returns a single route with full price history
    - `routes` returns all tracked routes with status
    - _Requirements: 5.1, 5.2, 5.3_

  - [x] 6.4 Add `fulfillTrip(tripId: Int!)` mutation
    - Call `TripService.fulfill_trip()`, return updated TripRequestType
    - _Requirements: 2.1_

  - [x] 6.5 Update existing `trips` query to filter by status "active"
    - Ensure only active contracts appear in the main trips list
    - _Requirements: 2.3_

- [x] 7. Build frontend ContractHistory page and route views
  - [x] 7.1 Add GraphQL queries and mutations to `frontend/src/graphql/queries.ts`
    - Add `GET_FULFILLED_TRIPS` query
    - Add `GET_ROUTE` and `GET_ROUTES` queries
    - Add `FULFILL_TRIP` mutation
    - _Requirements: 4.1, 5.1_

  - [x] 7.2 Create `frontend/src/pages/ContractHistory.tsx`
  ...
  - [x] 7.3 Create `frontend/src/components/RoutePriceHistory.tsx`
  ...
  - [x] 7.4 Add "Mark as Fulfilled" button to TripDetail page
    - Add button that calls `FULFILL_TRIP` mutation
    - On success, redirect to contract history or show confirmation
    - _Requirements: 2.1_

  - [x] 7.5 Add navigation to ContractHistory page in App routing
    - Add route in `App.tsx` for `/history`
    - Add navigation link in TripList header
    - _Requirements: 4.1_

- [x] 8. Checkpoint - Ensure frontend builds and backend integration works
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Create migration script for existing data
  - [x] 9.1 Create `scripts/migrate_routes.py`
...
- [x] 10. Create new trip contracts (PHX→ATL and PHX→KIX)
  - [x] 10.1 Add seed script or instructions to create PHX→ATL contract
  ...
  - [x] 10.2 Add seed script or instructions to create PHX→Japan contracts
...
- Backend uses Python (SQLAlchemy, Strawberry GraphQL)
